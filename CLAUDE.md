...
(control de gastos personal: control semanal, cuotas y metas de ahorro), cómo funciona,
dónde está cada cosa, el modelo de datos en Supabase, los cálculos derivados (gasto
semanal, presupuesto con arrastre), los flujos y —crítico— los **puntos sensibles** y las
...
| Complejo | `claude-opus-4-8` | Arquitectura, refactors grandes, seguridad, diseño de sistema |
| Muy Complejo | `claude-fable-5` | Cosas extremadamente complejas o planes muy elaborados |

Al despachar subagentes (`Agent` tool): usar **siempre el modelo más barato** que pueda
...
- Subtareas independientes → subagentes paralelos con `Agent`.
...
| Infra / config (Vercel, env, tooling) | Haiku |
| Docs / comentarios | Haiku |
| Componente UI nuevo (shadcn/Tailwind) | Sonnet |
| Cálculo de negocio (gasto semanal, cuotas, arrastre) | Opus (revisar números) |
